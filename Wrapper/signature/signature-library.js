  ...
  return {
    ...
    startSigning: function (callback) {
      try {
        ...
        console.log(
          `Found ${elements.length} signature placeholder(s), ${invalidCount} invalid`
        );
        return {
          success: true,
    ...
    getAllSignatures: function () {
      ...
      console.log(
        `Found ${signatures.length} completed signature(s) out of ${elements.length} placeholder(s)`
      );
      return signatures;
    },
    ...
    getSignatureById: function (signatureId) {
      ...
      const element = document.querySelector(
        `.${SIGNATURE_CLASS}[data-signature-id="${signatureId}"]`
      );
      if (!element) {
    ...
    validateElement: function (element) {
      ...
      if (!element.classList.contains(SIGNATURE_CLASS)) {
        return {
          valid: false,
          message: `Element does not have class: ${SIGNATURE_CLASS}`,
        };
      }
      ...
      if (!signatureId) {
        return {
          valid: false,
          message: "Element missing data-signature-id attribute",
        };
      }
